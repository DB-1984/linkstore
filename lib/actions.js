...
export async function updatePassword(formData) {
  try {
    ...
    // 2. Use Schema Method to check current password
...
export async function requestPasswordReset(formData) {
  ...
  // Using your existing schema fields
  user.resetPasswordToken = token;
  user.resetPasswordExpires = Date.now() + 3600000; // 1 hour

  await user.save();

  // Send the email (We'll build this in lib/mail.js next)
...
export async function confirmPasswordReset(formData) {
  ...
  // Set new password (Mongoose hashes this in pre-save)
  user.password = newPassword;

  // IMPORTANT: Clear the token so it can't be used again
